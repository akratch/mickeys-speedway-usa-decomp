 ...
 * 0.070 and all 119 flag combinations miss. Reordering the independent height
 * update and preserving a four-byte pad yields the exact 48-byte frame and
 * post-call schedule; two of 82 words still differ, first at +0xEC, because
 * IDO spills the call-live node at sp+0x24 instead of retail's sp+0x1C. A
 * bounded two-worker permuter batch improved cost only by making the current
 * wrap assignment unconditional, so that semantically invalid form was
 * rejected. */
#ifdef NON_MATCHING
