...
Acmd *n_alFxPull(s32 sampleOffset, Acmd *p, s32 arg2)
{
    ...
    for (j = 0; j <= D_800D7DC4[arg2]; j++) {
        ...
        for (i = 0; i < r->section_count; i++) {
            ...
            if (D_800D7DC6[arg2] && D_800D7DC4[arg2]) {
                d->ffcoef = -d->ffcoef;
                d->fbcoef = -d->fbcoef;
            }
...
Acmd *_n_loadOutputBuffer(ALFx *r, ALDelay *d, s32 arg2, s32 buff, Acmd *p)
{
    ...
    if (d->rs) {
...
#else
        {
            ...
            tmp = buff >> 8;
            n_aResample(ptr++, osVirtualToPhysical(d->rs->state[arg2]), d->rs->first, ratio,
