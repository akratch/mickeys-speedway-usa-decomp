/*
 * libultra PI device-manager dispatcher.
 *
 * Jet Force Gemini's built `libultra/src/io/devmgr.c.o` is the unique
 * whole-object match for Mickey's 0x410-byte text and 0x20-byte jump table.
 * Flags: -O2 -g3 -mips2 -32.
 *
 * PROVENANCE: the body and 64DD SDK names are adapted from Jet Force Gemini's
 * `libultra/src/io/devmgr.c`, a permitted published decomp source under
 * docs/CLEANROOM.md. Mickey's bytes decide the local layout and flags.
 */
