/*
 * libultra: the Controller Pak's identity block and inode-table I/O.
 *
 * ROM 0x6DE90-0x6EA30 (VRAM 0x8006D290). Byte-identical to Jet Force Gemini's
 * built libultra `io/contpfs.c` object, so the file boundary is measured
 * rather than guessed -- see the provenance note in symbol_addrs.us.txt.
 *
 * Flags: -O2 -g3 -mips2 -32.
 *
 * DEVIATION FROM THE REFERENCE: the three inode-cache objects are `extern`
 * here rather than defined in this unit, because Mickey's .data and .bss are
 * still one unsplit subsegment (docs/modules.md section 6.3).
 *
