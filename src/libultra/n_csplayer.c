...
#include "n_audio/cseq.h"
#include "n_audio/n_cseqp.h"
...
s32 g_CspTimeLookup[] = {
	...
	9900000,   10000000,  11000000,  12000000,  /* 10-12 seconds */
	13000000,  14000000,  /* 13-14 seconds */ 15000000,  16000000,  /* 15-16 seconds */
	17000000,  18000000,  /* 17-18 seconds */ 19000000,  21000000,  /* 19-21 seconds */
	22000000,  24000000,  /* 22-24 seconds */ 26000000,  28000000,  /* 26-28 seconds */
	30000000,  32000000,  /* 30-32 seconds */ 34000000,  37000000,  /* 34-37 seconds */
	39000000,  42000000,  /* 39-42 seconds */ 45000000,  49000000,  /* 45-49 seconds */
	50000000,  55000000,  /* 50-55 seconds */ 60000000,  65000000,  /* 60-65 seconds */
	70000000,  75000000,  /* 70-75 seconds */ 80000000,  85000000,  /* 80-85 seconds */
	90000000,  95000000,  100000000, 105000000, /* 90-105 seconds */
	...
	do {
		...
		case (AL_SEQP_STOPPING_EVT):
			if (seqp->state == AL_PLAYING) {
				...
				n_alEvtqFlushType(&seqp->evtq, AL_TREM_OSC_EVT);
				n_alEvtqFlushType(&seqp->evtq, AL_VIB_OSC_EVT);
...
void func_8005E7A8(N_ALCSPlayer *seqp, u8 channel)
{
	N_ALVoiceState *vs;
	s16 sp2a;
	...
	for (vs = seqp->vAllocHead; vs != 0; vs = vs->next) {
		if (vs->channel == channel) {
			...
			n_alSynSetLpf_gain(&vs->voice, sp2a);

			if (sp2a) {
				n_alSynSetLpf_freq(&vs->voice, alSemitones2Ratio(vs->key - vs->sound->keyMap->keyBase + sp29) * 440 * sp24);
			}
		}
	}
}
...
void __n_CSPHandleMIDIMsg(N_ALCSPlayer *seqp, N_ALEvent *event)
{
	...
	case (AL_MIDI_NoteOn):
		...
		if (vel != 0) /* a real note on */ {
			...
			if ((chanstate->notemesgflags & 1) && seqp->queue) {
				osSendMesg(seqp->queue, (OSMesg)((D_800D7D80[chan] & 0xffffff00) | (chanstate->notemesgflags >> 2)), OS_MESG_NOBLOCK);
			}
	...
	case (AL_MIDI_ControlChange):
		...
		case (AL_MIDI_SETFADEINC_CTRL):
			...
			if (sp67 != sp66) {
				n_alEvtqPostEvent(&seqp->evtq, event, seqp->uspt * 100);
			}
