/*
 * libultra: read or write one file's pages through the pak's inode chain.
 *
 * ROM 0x6CE70-0x6D2D0 (VRAM 0x8006C270). Byte-identical to Jet Force Gemini's
 * built libultra `io/pfsreadwritefile.c` object, so the file boundary is
...
s32 osPfsReadWriteFile(OSPfs *pfs, s32 file_no, u8 flag, int offset, int size_in_bytes,
    ...
    if (flag == PFS_WRITE && (dir.status & DIR_STATUS_OCCUPIED) == 0) {
        ...
        SET_ACTIVEBANK_TO_ZERO();
        ERRCK(__osContRamWrite(pfs->queue, pfs->channel, pfs->dir_table + file_no, (u8 *)&dir,
                               FALSE));
    }
