/*
 * libultra: create a directory entry and chain enough free pages behind it.
 *
 * ROM 0x6D2D0-0x6D750 (VRAM 0x8006C6D0). Byte-identical to Jet Force Gemini's
 * built libultra `io/pfsallocatefile.c` object, so the file boundary is
...
s32 osPfsAllocateFile(OSPfs *pfs, u16 company_code, u32 game_code, u8 *game_name, u8 *ext_name,
    ...
    for (bank = 0; bank < pfs->banks; bank++) {
        ...
        ERRCK(__osPfsDeclearPage(pfs, &inode, file_size_in_pages, &start_page, bank, &decleared,
                                 &last_page));
...
s32 __osPfsDeclearPage(OSPfs *pfs, __OSInode *inode, int file_size_in_pages, int *first_page,
                       u8 bank, int *decleared, int *last_page) {
    int j;
    int spage;
    int old_page;
    s32 ret = 0;
