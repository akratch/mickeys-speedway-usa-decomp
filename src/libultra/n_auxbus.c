...
Acmd *n_alAuxBusPull(s32 sampleOffset, Acmd *cmdptr, s32 fxBus, s32 *numpulls) {
    ...
    *numpulls = 0;
    ...
    for (i = 0; i < bus->sourceCount; i++) {
        if (sources[i]->vvoice && sources[i]->vvoice->fxBus == fxBus && sources[i]->vvoice->pvoice &&
            sources[i]->vvoice->pvoice->unk8c >= 64) {
    ...
    for (i = 0; i < bus->sourceCount; i++) {
        if ((sources[i]->vvoice && sources[i]->vvoice->fxBus == fxBus &&
             (sources[i]->vvoice->pvoice == NULL || sources[i]->vvoice->pvoice->unk8c < 64)) ||
