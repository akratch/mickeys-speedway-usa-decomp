/*
 * libultra: which controller channels have a Controller Pak inserted.
 *
 * ROM 0x6C450-0x6C750 (VRAM 0x8006B850). Byte-identical to Jet Force Gemini's
 * built libultra `io/pfsisplug.c` object, so the file boundary is measured
 * rather than guessed -- see the provenance note in symbol_addrs.us.txt.
 *
 * Flags: -O2 -g3 -mips2 -32.
 *
 * DEVIATION FROM THE REFERENCE: the SDK source defines `__osPfsPifRam` here.
 * It is `extern` in this tree because Mickey's .bss is still one unsplit
 * subsegment (docs/modules.md section 6.3); the emitted .text is unaffected.
 *
