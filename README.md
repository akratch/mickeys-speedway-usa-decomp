...
typed by hand. `gmake check-scoreboard` needs a linked ELF, which needs a
baserom, which CI can never have — so CI runs
[`scoreboard.yml`](.github/workflows/scoreboard.yml)'s `--check-partial`
instead: the non-ELF-derived figures and the block's own arithmetic, not the
match counts themselves. See
[`docs/CONTRIBUTING.md`](docs/CONTRIBUTING.md#checks) for what that does and
doesn't cover.)*
