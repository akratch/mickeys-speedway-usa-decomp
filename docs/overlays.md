...
`overlay 34 +0x040C..+0x0540` (`overlay34UpdateRecords`) is **308 exact C bytes / 77 words**. Moving the countdown initializer into the live-record branch makes untouched configured IDO emit the retail `0x30` frame and exact instruction stream. All seven runtime records agree by offset, type, and effective identity; the linked owner, complete overlay, and full ROM are byte-identical.

JFG's public `src/overlays/o10/overlay_10.c:sparkUpdate` is assembly-only but is the closest measured structural counterpart. JFG maintainers can use Mickey's exact body as the update/remove scaffold at that insertion point; JFG's extra lifetime/fade behavior and different field layout remain project-specific reconstruction work.
