...
**Matching progress.** Eighteen functions currently compile exactly under
...
Mickey's field-base symbol. The 12-byte `levelGetNumber` global getter begins
the exact `level` work. The exact total is now 308 bytes.
