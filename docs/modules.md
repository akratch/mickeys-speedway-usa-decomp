...
VRAM `0x8006E820`–`0x80076110`, `0x78F0` bytes. **99 named subsegments,
including 98 measured whole-`.text` file boundaries plus one exact
...
The remaining unnamed code is `0x5F0`, **4.9% of the corridor**, in one
contiguous run:
...
against a single build; at 95.1% against five it is carrying more weight than
it has earned, and the plainest reading of `0x5F0` that five libultra builds do
...
Disassemble `0x74090`–`0x74680` before running the finder over anything else.
...
it, which is why all 98 whole-TU subsegments now carry measured boundaries.
