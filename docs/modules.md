...
VRAM `0x8006E820`–`0x80076110`, `0x78F0` bytes. **100 named subsegments,
including 99 measured whole-`.text` file boundaries plus one exact
function-tail carve, and 128 named functions**, all tier A. The yaml carries
...
The remaining unnamed code is `0x310`, **2.5% of the corridor**, in one
contiguous run:
...
It matches no object in any of the five reference builds, whole or
per-function: five negatives, two of them from byte-perfect builds.

**Read this run rather than mining it further.** "Unnamed code inside
the corridor is libultra-shaped" was a fair assumption at 78% identified
against a single build; at 97.5% against five it is carrying more weight than
it has earned, and the plainest reading of `0x310` that five libultra builds do
...
Disassemble `0x74090`–`0x743A0` before running the finder over anything else.
...
it, which is why all 99 whole-TU subsegments now carry measured boundaries.
