...
| `amTuneStop` | `0x18EC` / `0x30` | **tier B**: JFG and DKR agree on the official role; the tune-change block and tune-player stop call pin the identity | IDO 5.3, `-O2 -mips2 -32`; exact 12 object words and both global/call relocation identities |
| `amAmbientStop` | `0x191C` / `0x38` | **tier B**: JFG and DKR agree on the official role; the playing guard, ambient-ID reset, and ambient-player stop call pin the identity | IDO 5.3, `-O2 -mips2 -32`; exact 14 object words and all global/call relocation identities |
| `amTuneGetSeqNo` | `0x1954` / `0x3C` | **tier B**: JFG and DKR agree on the official role; the current-tune guard and tune-player `AL_PLAYING` state check pin the identity | IDO 5.3, `-O2 -mips2 -32`; exact 15 object words and all data relocation identities |
| `amAmbientGetSeqNo` | `0x1990` / `0x0C` | **tier B**: JFG and DKR agree on the official return role; Mickey returns the same current-ambient global used by the play/stop pair | IDO 5.3, `-O2 -mips2 -32`; exact 3 object words and data relocation identity |
| `amTuneSetVolume` | `0x199C` / `0x6C` | **tier B**: JFG supplies the full body and official name; the clamp, saved base volume, scaled tune-player call, and update flag agree exactly | IDO 5.3, `-O2 -mips2 -32`; exact 27 object words and all data/call relocation identities |
| `amTuneSetGlobalVolume` | `0x1A08` / `0x5C` | **tier B**: JFG supplies the full body and official name; the global-volume clamp, saved scale, and recalculated tune-player call agree exactly | IDO 5.3, `-O2 -mips2 -32`; exact 23 object words and all data/call relocation identities |
| `amTuneGetVolume` | `0x1A64` / `0x0C` | **tier B**: JFG and DKR agree on the official return role; Mickey returns the base-volume global written by `amTuneSetVolume` | IDO 5.3, `-O2 -mips2 -32`; exact 3 object words and data relocation identity |
| `amAmbientSetVolume` | `0x1A70` / `0x4C` | **tier B**: JFG supplies the full body and official name; the saved relative volume and sound-global-scaled ambient-player call agree exactly | IDO 5.3, `-O2 -mips2 -32`; exact 19 object words and all data/call relocation identities |
| `amDittyPlay` | `0x1ABC` / `0x64` | **tier B**: JFG has the same exact boundary and sequence-table guard/current-ID/player-start role; `skeleton_scan.py` ranks it first at 0.571, not tier-A identity | IDO 5.3, `-O2 -mips2 -32`; exact 25 object words and all data/call relocation identities |
| `amDittyPlaying` | `0x1B20` / `0x54` | **tier B**: JFG has the same exact boundary and DKR supplies the official role; current-ID, enabled, and ambient-player-state guards agree exactly | IDO 5.3, `-O2 -mips2 -32`; exact 21 object words and all data relocation identities |
| `amSndStop` | `0x1B74` / `0x20` | **tier B**: JFG supplies the complete one-call body and official name; the target is below the skeleton oracle's 10-word confidence floor | IDO 5.3, `-O2 -mips2 -32`; exact 8 object words and call relocation identity |
| `amSndPlay` | `0x1B94` / `0x104` | **tier B**: JFG has the same exact boundary and direct-player call shape; DKR supplies the official role and `SoundData` interpretation | IDO 5.3, `-O2 -mips2 -32`; exact 65 object words and all data/call relocation identities |
| `amSndPlayDirect` | `0x1C98` / `0xAC` | **tier B**: JFG supplies the official name, parameter roles, range check, scaler, and direct-player call shape; Mickey's branch-likely form is four bytes shorter | IDO 5.3, `-O2 -mips2 -32`; exact 43 object words and all data/call relocation identities |
| `amSndSetVol` | `0x1D44` / `0xC0` | **tier B**: JFG and DKR agree on the official role; base-volume lookup, relative scaling, resident scaler, and volume-parameter call agree exactly | IDO 5.3, `-O2 -mips2 -32`; exact 48 object words and all data/call relocation identities |
| `amSndSetPitchDirect` | `0x1E2C` / `0x2C` | **tier B**: JFG and DKR agree on the official name and parameter role; the handle guard and pitch-parameter call agree exactly | IDO 5.3, `-O2 -mips2 -32`; exact 11 object words and call relocation identity |
| `amGetSfxCount` | `0x1E58` / `0x18` | **tier B**: JFG supplies the complete body and official name; the bank/instrument traversal and sound-count field agree exactly | IDO 5.3, `-O2 -mips2 -32`; exact 6 object words and data relocation identity |
| `amGetSfxSettings` | `0x1E70` / `0x38` | **tier B**: JFG supplies the complete body and official name; the optional table/size/count outputs and their globals agree exactly | IDO 5.3, `-O2 -mips2 -32`; exact 14 object words and all data relocation identities |
| `amSoundIsLooped` | `0x1EA8` / `0x60` | **tier B**: JFG and DKR agree on the official role and body; the sound-count bound, sound-array traversal, and infinite-decay test agree exactly | IDO 5.3, `-O2 -mips2 -32`; exact 24 object words and data relocation identity |
| `amTuneSetReverbOnOff` | `0x21F0` / `0x8` | **tier B**: JFG supplies the official name and identical no-op body at the corresponding post-sequence-control position | IDO 5.3, `-O2 -mips2 -32`; exact 2 object words and no relocations |
| `func_800015F8` | `0x21F8` / `0x10` | **tier D**: direct write of one to the resident audio flag; no external name is asserted | IDO 5.3, `-O2 -mips2 -32`; exact 4 object words and data relocation identity |
