...
**Matching progress.** Four leaf functions currently compile exactly under
...
`joySetSecurity` store. The 20-byte `joyDisable` masked-index store also
matches. Their HI16/LO16 pairs bind the original Mickey data symbols; owned
bytes, relocation identity and the linked ROM are exact (48 bytes total).
