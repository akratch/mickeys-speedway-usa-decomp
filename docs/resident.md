...
| `func_80022FD4` | `0x23BD4` | D — workbench `structure-mismatch`; the best `-Wab,-r4300_mul` candidate has the exact `0xB0` frame, 365/369 instructions and 217 positional differences from `+0x2C`. A volatile angle-product home removes 80 differences; the twelve-byte coordinate-home shift, four-instruction deficit, final Gfx schedule and downstream relocation alignment remain. |
...
| `0x8005716C` | `0x140` | no unique `hit.c` candidate | D; skeleton similarity peaks at 0.156, and the fresh exact-sibling vector-alias route worsens to 82 words/64 differences. The 80-word candidate remains best at 18 FP register/schedule words, first `+0x54`; workbench `mixed`/`constant-audit` sees target spill `sp+4` versus candidate `sp+0` |
