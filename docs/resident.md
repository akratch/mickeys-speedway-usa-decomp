...
| `func_80012574` | 0xE4 / 57 words | JFG's assembly-only `trackSphereIntersect` supplies tier-D collision structure; the Mickey reconstruction is retained under `NON_MATCHING`. | `+0x50`: workbench `register-permutation`; exact frame, 57-word schedule, and `sqrtf` relocation, but IDO swaps the `f14`/`f18` projection webs. Declaration, lifetime, volatile, and ABI probes did not move it; forced-color tooling is unavailable. |
| `func_8000FAE0` | 0xF8 / 62 words | DKR `get_level_segment_index_from_position` and JFG `trackGetBlock` establish the segment scan; the Mickey reconstruction is retained under `NON_MATCHING`. The stock `-O2 -mips2 -32` flag basin is uniquely best after the 119-mode sweep. | `+0x1C`: workbench `structure-mismatch`; exact 62-word shape and two relocations, but IDO colors the segment count into `$a0` instead of target `$t0`, cascading through the bound checks. Further source/goto and declaration-order forms canonicalize to 43 positional differences; no safe new lever remains. |
| `func_8000D018` | 0x154 / 85 words | JFG's assembly-only `func_800135E0` establishes the camera/update skeleton; Mickey's typed field reconstruction is retained under `NON_MATCHING`. A controlled three-`f32` trampoline declaration makes the function exact under `-O2 -mips2 -32`. | `+0xF4`: the canonical old-style `TrapDanglingJump` declaration expands the three-float call to 89 words, frame `0x20`, and 34 positional differences; the typed declaration is exact but IDO rejects the same symbol's existing integer/pointer call sites. No safe dual-ABI declaration mechanism remains. |
...
|---|---:|---|
| `func_80024834` | `0x25434` | D — retained Mickey auto-name; JFG `camReversePoint` role. Plateau: exact 65-word size and nine relocations, 25 masked words differ from `+0x0`; target frame `0x38`, candidate `0x40`; the full flag lattice, 40-minute permuter, and 2026-08-26 volatile-index probe found no exact source. |
...
| `0x80050BF4` | `0x15C` | `animseqInit` | D; fresh m2c route retains the exact 87-word/0x18-frame shape with 15 words from `+0x34`; typed per-table cursors worsen to 23, while the flag sweep leaves the same `structure-buckets` schedule. |
...
| `0x80050E9C` | `0x168` | `animseqFreeGroup` | B; fresh raw-cursor/lifetime route is a `structure-mismatch` plateau at 54 words versus the typed m2c candidate's 46, first intrinsic `+0x64`; IDO still carries the scroll boundary instead of rematerializing the lockon base, so retain the 89/90-instruction candidate. |
...
| `0x80055F64` | `0x16C` | no unique `hit.c` candidate | D; fresh m2c typed-home route retains exact 91 instructions, `0x48` frame, and relocations with 3 constant-only words from `+0xA0` (`sp+0x1C` target versus `sp+0x18`); a volatile aggregate is unchanged and an indexed home widens the frame, so retain the `constant-audit` candidate. |
...
| `0x8005716C` | `0x140` | no unique `hit.c` candidate | D; fresh explicit normal-component capture worsens the candidate to 58 words, while the 80-word body remains best at 18 FP register/schedule words, first `+0x54`; workbench `mixed`/`constant-audit` still sees target spill `sp+4` versus candidate `sp+0`. |
