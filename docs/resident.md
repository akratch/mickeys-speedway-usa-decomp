...
| `func_80024978` | `0x25578` | D — JFG `camCopyOrthoMatrix` role/loop; camera now owns the measured `0x10`-byte `D_80079F48`–`D_80079F54` input section. Workbench remains `structure-mismatch`: best C candidate 84 instructions versus 83, first register divergence `+0x8`, with raw-table sentinel and third-coefficient scheduling unresolved; assembly remains canonical. |
