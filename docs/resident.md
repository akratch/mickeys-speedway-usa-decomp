...
| `0x80019DE8` | `0xFC` | `lightSetObjectLight` | tier-D boundary; bounded reproof preserves configured C at 64 versus target 63 words, exact frame `0x38`, 19/63 positional matches, first `+0x44`; the extra executable word shifts all three correct relocation identities by four bytes. All 119 flags and two valueDelta-first forms are nonexact; the proc-22 trace provides no source attribution. Assembly remains canonical until a natural structural form removes that word. |
...
| `func_8000DDE4` | 0x1D8 / 118 words | Matched C: the natural `recordIndex`/`passCount` lifetime merge closes 118/118 words with exact frame `0x28`, exact `runlinkIsModuleLoaded` and `TrapDanglingJump` relocations, runtime identity through ORT 1315, linked owned range, and full ROM. | Complete; the 119-flag lattice and donor scan were nonexact, while the first trace-supported natural lifetime form matched. |
...
| `func_8000DB34` | 0x2B0 / 172 words | Evidence D: Mickey's m2c control-flow draft and resident offsets reconstruct the inverse segment-order table, descending object scan, integer AABB test, and eight-byte result records. | Candidate: structure-mismatch, 267/172 instructions, 267 differing words, first `+0x0`, frame `-0x188` versus `-0x190`; the input-map loop is over-unrolled and local/register shape remains unresolved. |
...
| `func_8000E5EC` | 0x334 / 205 words | Evidence D: Mickey's reconstruction and DKR's level-geometry renderer establish visible-segment selection, visibility-map setup, particle passes, and dispatch. | Candidate: structure-mismatch, 284/205 instructions, 284 differing words, first `+0x0`, frame `-0x78` versus `-0xD8`; stack-byte and call-signature structure remain unresolved. |
...
- `func_80028FCC`: fresh configured full-TU C with canonical
  ...
  `0x18`, first `+0x1C`, and all three calls at `+0x14/+0x30/+0x4C`. The
  natural shared-result/epilogue probe regressed to 25 words and moved the
  latter two calls, so it did not unlock another lattice or trace. ORT 663 has
  no authenticated caller; linked equality proves fallback only.
...
| `func_800470B0` | `FxCone` bytes `+14..17`, pointer/output words `+8,+C,+10,+15`; generated vertex records; `func_8002A8BC/C0`. | Evidence D candidate: structure-mismatch, 150/149 instructions, 121 differing words, first `+0x44`, exact `-0x168` frame, and seven relocation-symbol sites differ; not shape-exact. |
...
| `0x8001C114` | `0x1B0` | `func_8002F0E8` | D + Mickey-derived exact C: 108/108 words, frame `0x18`, and all six static relocation offsets, types, and identities agree. Grouping the zero index and array cursor initializations in one ordered expression gives IDO the target allocation and late schedule without forcing compiler output. The function maintains a camera override by rejecting an out-of-range tracked object and selecting an eligible replacement. JFG's corresponding `func_8002F0E8` remains assembly-only, so this body is a direct clean-room candidate at that insertion point. |
