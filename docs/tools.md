...
| `tools/run_logged.py` | Runs one build command with complete output under `build/`, prints one compact PASS/FAIL receipt, and shows only a bounded tail on failure. `gmake verify` uses it for candidate rebuilds and the final ROM build. | `gmake verify` |
